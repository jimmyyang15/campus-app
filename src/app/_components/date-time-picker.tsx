...
function Calendar(props: CalendarProps<DateValue>) {
  ...
  const state = useCalendarState({
    ...props,
    locale,
    createCalendar:createCalendar as any,
  });
  const {
    calendarProps,
...
function CalendarGrid({ state, ...props }: CalendarGridProps) {
  ...
  const weeksInMonth = getWeeksInMonth(state.visibleRange.start as any, locale);
        ...
        {[...new Array(weeksInMonth).keys()].map((weekIndex) => (
          <tr className="mt-2 flex w-full" key={weekIndex}>
            {state
              ...
              .map((date, i) =>
                date ? <CalendarCell key={i} state={state} date={date as any} /> : <td key={i} />,
              )}
          </tr>
        ))}
      </tbody>
...
>(({ jsDate, onJsDateChange, showClearButton = true, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      <DateField {...fieldProps} value={currentValue() as any}  />
